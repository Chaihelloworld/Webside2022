...
function Content() {
  ...
  const fetchdata = async () => {
    ...
    try {
      const result = await axios.get(
        `https://www.serverwebp-api.com/api/resource/report?year=${2022}`
      );
  ...
  return (
    ...
    <div>
    {" "}
          <BsSlashLg color="#4a5568" />{" "}
          <Link href={"/report"}>
            <a style={{ color: "#797979", fontSize: 14 }}>
              ค่า kg CO2 -eq ที่งหมดในปี {'2564'} เท่ากับ{" "}
              {valueCO2.toLocaleString()} KgCO2
            </a>
          </Link>
    </div>
      <div>
      {" "}
          <BsSlashLg color="#3182ce" />{" "}
          <Link href={"/report"}>
            <a style={{ color: "#797979", fontSize: 14 }}>
              tonne CO2 ที่งหมดในปี {'2564'} เท่ากับ {valuetonne.toLocaleString()} TCO2
            </a>
          </Link>
      </div>
      {/* <ListGroup>
        <ListGroupItem>
          ...
          <a
            ...
            style={{ color: "#2e2d2d", fontSize: 14, fontWeight: 650 }}
          >
            {" "}
            <BsBarChartLine style={{ transform: "translate(0px,-2px)" }} />{" "}
            ข้อมูลทรัพยากร
          </a>
        </ListGroupItem>
        <ListGroupItem>
          ...
          <Link href={"/report"}>
            <a style={{ color: "#797979", fontSize: 14 }}>
              ค่า kg CO2 -eq ที่งหมดในปี {date} เท่ากับ{" "}
              {valueCO2.toLocaleString()}
            </a>
          </Link>
        </ListGroupItem>
        <ListGroupItem>
          ...
          <Link href={"/report"}>
            <a style={{ color: "#797979", fontSize: 14 }}>
              tonne CO2 ที่งหมดในปี {date} เท่ากับ {valuetonne.toLocaleString()}
            </a>
          </Link>
        </ListGroupItem>
      </ListGroup> */}
